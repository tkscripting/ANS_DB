...
const fileExtInput = document.getElementById("cardFileExt");
...
submitBtn.onclick = async () => {
  ...
  const fileExt = fileExtInput.value.trim().replace(/^\./, "");
...
async function renderCard(id, status, data) {
  ...
  if (auth.currentUser) {
    ...
    editBtn.onclick = async () => {
      ...
      fileExtInput.value = data.fileExt || "";
      modal.classList.remove("hidden");
...
function createDownloadButton(docId) {
  ...
  btn.onclick = async () => {
    try {
      ...
      const fileExt = data?.fileExt?.trim() || "js";
      ...
      await handleDownload(url, fileExt);
    } catch (err) {
...
async function handleDownload(url, fileExt = "js") {
  try {
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}: ${res.statusText}`);
    const blob = await res.blob();

    let filename = decodeURIComponent(url.split("/").pop() || "file");
